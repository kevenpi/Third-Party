...
type SaveBubbleOptions = {
  voiceConversationId?: string;
  audioClipPath?: string;
  transcriptSnippet?: string;
  highlightPoints?: TimelineBubble["highlightPoints"];
};

function isValidConversationSession(session: RecordingSession, durationSec: number): boolean {
  if (durationSec < 8) return false;
  if (session.faceIdentification && session.faceIdentification.confidence !== "low") {
    return true;
  }
  const ev = session.evidence;
  if (!ev) {
    // Backward compatibility: if evidence is missing, allow medium-length sessions.
    return durationSec >= 20;
  }
  const avgTranscriptConfidence =
    ev.samples > 0 ? ev.transcriptConfidenceSum / ev.samples : 0;
  const transcriptRichEnough =
    ev.transcriptWords >= 8 || (ev.transcriptWords >= 4 && avgTranscriptConfidence >= 0.2);
  const speechRichEnough = ev.legibleFrames >= 3 && durationSec >= 8;
  const sustainedSpeechEvidence =
    ev.samples >= 4 &&
    ev.legibleFrames / Math.max(1, ev.samples) >= 0.45 &&
    durationSec >= 10;
  return transcriptRichEnough || speechRichEnough || sustainedSpeechEvidence;
}

export async function saveTimelineBubbleFromSession(
  session: RecordingSession,
  options?: SaveBubbleOptions
  ...
  const date = localDateStr(new Date(session.startedAt));
  ...
  // Compute biometric-driven values. Prefer persisted session samples, then analyzed biometric data.
  ...
  const analyzedBio = await loadBiometricData(session.id);
  if (analyzedBio?.hrTimeline?.length) {
    const avgStress =
      analyzedBio.hrTimeline.reduce((sum, point) => sum + point.stress, 0) /
      analyzedBio.hrTimeline.length;
    const avgHr =
      analyzedBio.hrTimeline.reduce((sum, point) => sum + point.hr, 0) /
      analyzedBio.hrTimeline.length;
    realCortisol = Math.min(1, avgStress / 100);
    realHeartRate = Math.min(1, avgHr / 120);

    const positives = analyzedBio.messageCorrelations.filter(
      (corr) => corr.annotation?.type === "positive"
    ).length;
    const negatives = analyzedBio.messageCorrelations.filter(
      (corr) => corr.annotation?.type === "missed_bid" || corr.annotation?.type === "observation"
    ).length;
    const recoveryBonus = Math.max(0, 0.2 - Math.min(0.2, analyzedBio.recovery.minutes * 0.01));
    realMeaningfulness = Math.max(
      0,
      Math.min(1, 0.5 + (positives - negatives) * 0.08 + recoveryBonus - realCortisol * 0.2)
    );
  }
  ...
  const previous = existing.find((b) => b.sessionId === session.id);
  ...
  const bubble: TimelineBubble = {
    ...
    voiceConversationId: options?.voiceConversationId ?? previous?.voiceConversationId,
    audioClipPath:
      options?.audioClipPath ??
      previous?.audioClipPath ??
      session.clipPaths[session.clipPaths.length - 1],
    transcriptSnippet: options?.transcriptSnippet ?? previous?.transcriptSnippet,
    highlightPoints: options?.highlightPoints ?? previous?.highlightPoints
  };
...
export async function getBubblesForDate(date: string): Promise<TimelineBubble[]> {
  ...
  try {
    ...
    return list.filter((bubble) => bubble?.sessionId && !String(bubble.sessionId).startsWith("seed_"));
  } catch {
    return [];
  }
}
...
export async function listTimelineDates(): Promise<string[]> {
  ...
  for (const date of dates) {
    ...
    if (bubbles.length > 0) out.push(date);
  }
  return out;
}
