...
async function stopActiveSession(state: ConversationAwarenessState): Promise<RecordingSession | null> {
  ...
  await upsertRecordingSession(endedSession);
  return endedSession;
}
...
export async function ingestAwarenessSignal(
  ...
  if (stopEvaluation.isConversation) {
    ...
    const endedSession = await stopActiveSession(state);
    state.isRecording = false;
    state.activeSessionId = undefined;
