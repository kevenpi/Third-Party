...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      // Look for voice pipeline conversations that match this session's timeframe
      ...
      for (const convo of conversations) {
        ...
        if (segments.length > 0) {
          // Check if the conversation overlaps with our session
          ...
          if (timeDiff < 5 * 60 * 1000) {
            // Within 5 minutes — likely the same conversation
    ...
    if (biometricData?.messageCorrelations?.length) {
